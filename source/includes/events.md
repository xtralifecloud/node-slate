...
function SendEvent()
{
    ...
    clan.withGamer(gamer).events("private").send("587f5d844877a1734ec079e6", evt, function(error, sendEventRes)
    {
      if(error)
...
This function returns the JSON you sent to the other user, as well as a `id` key whose value is an identifier generated by
XtraLife (optionally used for "acking" reception of the message).
